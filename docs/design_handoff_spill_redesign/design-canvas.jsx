...
// Also peeks through pure function-component wrappers whose render returns
// a DCArtboard (e.g. <MarkCard …/> → <DCArtboard …>…</DCArtboard>) — they
// keep their factory shape, but the walk sees the underlying DCArtboard.
function dcFlatten(children) {
  const out = [];
  React.Children.forEach(children, (c) => {
    if (c && c.type === React.Fragment) { out.push(...dcFlatten(c.props.children)); return; }
    let r = c;
    let depth = 0;
    while (r && typeof r.type === 'function'
      && r.type !== DCArtboard && r.type !== DCSection && depth < 4) {
      let next;
      try { next = r.type(r.props); } catch { break; }
      if (!next || next === r) break;
      r = next;
      depth++;
    }
    out.push(r && r.type === DCArtboard ? r : c);
  });
  return out;
}
...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  dcFlatten(children).forEach((sec) => {
    ...
    dcFlatten(sec.props.children).forEach((ab) => {
      // Resolve wrapper components that return a DCArtboard (e.g.
      // <MarkCard …/> whose render is <DCArtboard …>…</DCArtboard>).
      // Pure factory functions only — no hooks.
      let resolved = ab;
      let depth = 0;
      while (resolved && resolved.type !== DCArtboard
        && typeof resolved.type === 'function' && depth < 4) {
        try { resolved = resolved.type(resolved.props); }
        catch { resolved = null; break; }
        depth++;
      }
      if (!resolved || resolved.type !== DCArtboard) return;
      const aid = resolved.props.id ?? resolved.props.label;
      if (aid) abs.push([aid, resolved]);
    });
