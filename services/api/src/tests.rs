...
async fn invalid_invitee_role_does_not_create_retro(pool: sqlx::PgPool) {
    ...
    let overview: Value =
        serde_json::from_slice(&to_bytes(response.into_body(), usize::MAX).await.unwrap()).unwrap();
    assert_eq!(overview["retros"].as_array().unwrap().len(), 0);
}
...
async fn host_can_update_retro_title_and_schedule(pool: sqlx::PgPool) {
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("content-type", "application/json")
                .body(Body::from(
                    r#"{"title":"Cleared metadata","scheduled_at":"","cover_gif_url":"","cover_gif_alt_text":""}"#,
                ))
                .unwrap(),
        )
        .await
    ...
    let cleared: Value =
        serde_json::from_slice(&to_bytes(response.into_body(), usize::MAX).await.unwrap()).unwrap();
    assert_eq!(cleared["retro"]["title"], "Cleared metadata");
    assert_eq!(cleared["retro"]["scheduled_at"], Value::Null);
    assert_eq!(cleared["retro"]["cover_gif_url"], Value::Null);
    assert_eq!(cleared["retro"]["cover_gif_alt_text"], Value::Null);
