  ...
  return (
    <div className="flex items-center gap-1" data-spill-no-drag>
      <form action={removeVoteAction}>
        ...
        <button className={cardButtonClass} disabled={card.current_user_vote_count <= 0} style={{ "--card-button-fg": color } as CSSProperties} type="submit">-</button>
      </form>
      ...
      <form action={castVoteAction}>
        <input name="retro_id" type="hidden" value={board.retro.id} />
        <input name="card_id" type="hidden" value={card.id} />
        <button className={cardButtonClass} disabled={board.voting.votes_remaining <= 0} style={{ "--card-button-fg": color } as CSSProperties} type="submit">+</button>
      </form>
    </div>
  );
}
